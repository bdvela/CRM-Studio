...
import { DollarSign, Plus, Search, TrendingUp, TrendingDown, Receipt, ArrowRight, CalendarDays, UserRound, Link2, ClipboardList, Clock, PiggyBank } from 'lucide-react';
...
type TabId = 'registrar' | 'pendientes' | 'resumen' | 'comisiones';
...
function PaymentDetailModal({ payment, appointment, client, onClose, onGoToAppointment, onGoToClient }: {
  payment: Payment; appointment: any | null; client: Client | null;
  onClose: () => void; onGoToAppointment: () => void; onGoToClient: () => void;
}) {
  return (
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-violet-700">
                  <UserRound className="size-4" /><p className="text-sm font-semibold">Clienta relacionada</p>
                </div>
              ...
              <Button variant="outline" size="sm" onClick={onGoToClient} className="shrink-0">Ver clienta<ArrowRight className="size-4 ml-1" /></Button>
            </div>
          </div>
        )}
        {appointment && (
          <div className="rounded-2xl border border-salon-100 bg-salon-50/60 p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-salon-700">
                  <CalendarDays className="size-4" /><p className="text-sm font-semibold">Cita relacionada</p>
                </div>
              ...
              <Button variant="outline" size="sm" onClick={onGoToAppointment} className="shrink-0">Ver cita<ArrowRight className="size-4 ml-1" /></Button>
            </div>
            ...
            {appointment.appointment_services?.length > 0 && (
              <div className="space-y-2 pt-1">
                <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider flex items-center gap-1"><ClipboardList className="size-3.5" /> Servicios</p>
...
export default function PagosPage({ initialData }: {
  initialData?: { payments: Payment[]; appointments: any[]; clients: any[] };
}) {
  const { push } = useRouter();
  const [payments, setPayments] = useState<Payment[]>(initialData?.payments || []);
  const [appointmentsData, setAppointmentsData] = useState<any[]>(initialData?.appointments || []);
  const [clientsData, setClientsData] = useState<any[]>(initialData?.clients || []);
  const [ui, dispatchUI] = useReducer(uiReducer, initialData ? { ...UI_INIT, loading: false } : UI_INIT);
  ...
  async function load() {
    ...
    try {
      ...
      setAppointmentsData(a as any[]);
      setClientsData(c as any[]);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <>
      ...
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-1 p-1 bg-zinc-100 rounded-xl overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => dispatchUI({ activeTab: tab.id })}
              className={cn(
                'flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all whitespace-nowrap',
                ui.activeTab === tab.id ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
              )}
            >
              {tab.icon}
              {tab.label}
          ...
          <>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              <Card>
                <CardContent className="py-4 flex items-center gap-3">
                  <div className="size-10 rounded-xl bg-green-100 flex items-center justify-center"><TrendingUp className="size-5 text-green-600" /></div>
                  <div><p className="text-xs text-zinc-400">Ingresos</p><p className="text-lg font-bold text-green-600">{formatCurrency(totalIngresos)}</p></div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="py-4 flex items-center gap-3">
                  <div className="size-10 rounded-xl bg-red-100 flex items-center justify-center"><TrendingDown className="size-5 text-red-600" /></div>
                  <div><p className="text-xs text-zinc-400">Egresos</p><p className="text-lg font-bold text-red-600">{formatCurrency(totalEgresos)}</p></div>
                </CardContent>
              </Card>
              <Card className="col-span-2 lg:col-span-1">
                <CardContent className="py-4 flex items-center gap-3">
                  <div className="size-10 rounded-xl bg-salon-100 flex items-center justify-center"><DollarSign className="size-5 text-salon-600" /></div>
                  <div><p className="text-xs text-zinc-400">Ganancia neta</p><p className="text-lg font-bold text-salon-600">{formatCurrency(totalIngresos - totalEgresos)}</p></div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
              <div className="relative flex-1 w-full">
                ...
                <input type="text" placeholder="Buscar..." value={ui.search}
              ...
              <div className="flex gap-1 p-1 bg-zinc-100 rounded-xl">
                {['all', 'ingreso', 'egreso'].map((t) => (
                  <button key={t} onClick={() => dispatchUI({ filterType: t })}
                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${ui.filterType === t ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}>
            ...
            {ui.loading ? (
              <div className="space-y-3">{[1, 2, 3].map((i) => <div key={'skel-' + i} className="h-16 rounded-2xl bg-zinc-100 animate-pulse" />)}</div>
            ) : filtered.length === 0 ? (
              <Card>
              ...
              <div className="space-y-2">
                {filtered.map((payment) => (
                  <Card key={payment.id} className="cursor-pointer hover:shadow-sm transition-all" onClick={() => setSelectedPaymentId(payment.id)}>
                    <CardContent className="flex items-center gap-4 py-3.5">
                      <div className={`size-10 rounded-xl flex items-center justify-center flex-shrink-0 ${payment.type === 'ingreso' ? 'bg-green-100' : 'bg-red-100'}`}>
                        {typeIcons[payment.type]}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-sm truncate">{payment.concept}</p>
                          {payment.payment_kind && <Badge variant="default" className="text-xs">{PAYMENT_KIND_LABELS[payment.payment_kind]}</Badge>}
                        </div>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {(payment.appointment_id || payment.client_id) && (
                          <p className="text-[10px] text-zinc-400 mt-1 flex items-center gap-1 justify-end"><Link2 className="size-3" />Vinculado</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {form.type === 'ingreso' && (
            <>
              <Select label="Tipo de pago" value={form.payment_kind || ''} onChange={(value) => dispatchForm({ payment_kind: value as PaymentKind })} options={[
                { value: 'reserva', label: `Reserva (S/${DEPOSIT_AMOUNT})` }, { value: 'pago_completo', label: 'Pago completo (press-on)' }, { value: 'pago_final', label: 'Pago final (saldo)' },
              ]} />
              <Select label="Método de pago" value={form.payment_method || ''} onChange={(value) => dispatchForm({ payment_method: value as PaymentMethod })} options={[
                { value: '', label: 'Seleccionar...' }, { value: 'efectivo', label: 'Efectivo' }, { value: 'tarjeta', label: 'Tarjeta' },
                { value: 'transferencia', label: 'Transferencia' }, { value: 'yape_plin', label: 'Yape/Plin' },
              ]} />
            </>
          )}
          {form.type === 'egreso' && (
            <Select label="Categoría" value={form.category} onChange={(value) => dispatchForm({ category: value as PaymentCategory })} options={[
              { value: 'servicio', label: 'Servicio' }, { value: 'insumo', label: 'Insumo' }, { value: 'alquiler', label: 'Alquiler' },
              { value: 'marketing', label: 'Marketing' }, { value: 'comisiones', label: 'Comisiones' }, { value: 'otro', label: 'Otro' },
            ]} />
          )}
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" className="flex-1" loading={ui.submitting}>
              {!ui.submitting && <Receipt className="size-4 mr-1" />}{ui.submitting ? 'Guardando...' : 'Registrar'}
            </Button>
          </div>
        </form>
      </Modal>
      ...
      {selectedPayment && (
        <PaymentDetailModal
          payment={selectedPayment} appointment={selectedAppointment} client={selectedClient}
