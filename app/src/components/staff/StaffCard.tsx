...
export const StaffCard = memo(function StaffCard({ member, onEdit }: StaffCardProps) {
  ...
  return (
    <div
      ...
      className={`rounded-2xl border border-zinc-200 bg-white shadow-sm cursor-pointer hover:shadow-md hover:border-salon-300 transition-all border-t-4 ${!member.active ? 'opacity-60' : ''} ${isOwner ? 'ring-2 ring-amber-300' : ''}`}
