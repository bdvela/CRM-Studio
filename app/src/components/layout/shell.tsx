...
export function Sidebar() {
  ...
  return (
    <aside className={cn(
      'hidden md:flex flex-col border-r border-zinc-200 bg-white transition-all duration-300',
      collapsed ? 'w-20' : 'w-64'
    )}>
      {/* Logo */}
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-zinc-100">
      <div className="flex items-center justify-between px-4 md:px-8 py-4">
        {title && <h1 className="text-xl md:text-2xl font-semibold text-zinc-900">{title}</h1>}
