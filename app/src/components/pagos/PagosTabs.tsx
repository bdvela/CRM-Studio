...
export const PagosTabs = memo(function PagosTabs({ activeTab, onTabChange }: PagosTabsProps) {
  return (
    <div
      className="flex justify-center gap-1 p-1 bg-zinc-100 rounded-xl overflow-x-auto"
      role="tablist"
    ...
    >
      {TABS.map((tab) => (
        <button
          ...
          className={cn(
            'flex-1 flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors whitespace-nowrap',
            activeTab === tab.id
